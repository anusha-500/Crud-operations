<template>
  <div class="flex flex-row items-center h-16 bg-white w-full px-4 bottom-0">
    <div>
      <button
...
async function postData() {
  ...
  const { data: response } = await useAuthLazyFetch(
    'https://v1-orm-lib.mars.hipso.cc/notes/TASKS/1143',
    postoptions,
  )
  emit('add', response)
}
const button = props.uid ? 'Update' : 'Send'
</script>
